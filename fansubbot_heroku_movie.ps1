...
$rootDir = $pwd
$trackerFileName = "fansub-movies_tracker.csv"
$latestFileName = "fansub-movies_tracker_new.csv"

$logcontent = @"
**Fansub (Movie) script executed on :** $(Get-Date -UFormat "%B %d, %Y %T %Z")
"@
...
"@
    ...
    if($itemProperty.Name -match "Movie Title|Notes")
    {
        $inline = $false
    }
...
{
    $embedObject = [PSCustomObject]@{
        title       = "Fansubbot (movie) Encountered an Error"
...
try {
    Invoke-WebRequest -Uri "$fansubURL/export?gid=1148329835&format=csv" -OutFile "$rootDir\$latestFileName"
    (Get-Content "$rootDir\$latestFileName" | Select-Object -Skip 1) | Set-Content "$rootDir\$latestFileName"
}
catch {
    errorEmbed "Failed to Download new CSV (F) (Movie)"
...
else
{
    ...
    $diff = Compare-Object $trackerFile $latestFile -Property "Movie Title", "Best Release", "Alternate", "Notes"

    foreach($diffItem in $diff | Where-Object SideIndicator -eq "=>")
    {
        $type = "New Movie Release Found!"
        $embedArray.Clear()
        $fieldArray.Clear()
