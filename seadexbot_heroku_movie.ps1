...
$rootDir = $pwd
$trackerFileName = "seadex-movies_tracker.csv"
$latestFileName = "seadex-movies_tracker_new.csv"

$logcontent = @"
**Seadex (Movie) script executed on :** $(Get-Date -UFormat "%B %d, %Y %T %Z")
"@
...
{
    $embedObject = [PSCustomObject]@{
        title       = "Seadexbot (movie) Encountered an Error"
...
try {
    Invoke-WebRequest -Uri "$seadexURL/export?gid=1589605198&format=csv" -OutFile "$rootDir\$latestFileName"
    (Get-Content "$rootDir\$latestFileName" | Select-Object -Skip 2) | Set-Content "$rootDir\$latestFileName"
}
catch {
    errorEmbed "Failed to Download new CSV (Movie)"
